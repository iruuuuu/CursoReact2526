  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transform-all duration-300 overflow-hidden border border-gray-100 group">
        <div className="relative h-48 overflow-hidden">
            ...
            alt={plato.nombre}
            />
            <div className="absolute top right-4 bg-amber-100 px-3 rounded-full text-xs font-bold text-orange-600">
                {plato.categoria}
            </div>
        </div>
        <div  className="p-6">
            <div className = "flex justify-between mb-3">
                <h3 className="text-xl font-bold text-gray-800">{plato.origen}</h3>
                <span className ="text-sm font-medium text-gray-600 bg-gray-50 px-2 py-2 rounded">{plato.calorias}</span>
            </div>
        </div>
        <div>
            <p className="text-gray-600 font-semibold m-4">Ingredientes:</p>
